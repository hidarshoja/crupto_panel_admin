...
const WithdrawalChart = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      setReceivedData([]);
      setXData([]);
      const { formattedDateFrom, formattedDateTo } = formatDate(dateBirth, dateBirth);

      // تبدیل تاریخ‌ها به یکم ماه
      const newFormattedDateFrom = format(setDate(parseISO(formattedDateFrom), 1), 'yyyy-MM-dd');
      const newFormattedDateTo = format(setDate(parseISO(formattedDateTo), 1), 'yyyy-MM-dd');
      ...
      const url = `/admin/statistics/time-frame?start_date=${newFormattedDateFrom} 00:00:00&end_date=${finalFormattedDateTo} 23:59:59&transaction_type=-1&asset_id=${selectedWallet}${userId ? `&user_id=${userId}` : ''}`;
  ...
  const setChartValues = (chart) => {
    ...
    if (chart.length > 0) {
      ...
      chart?.map((items) => {
        ...
        setXData((prev) => [...prev, items.sum_withdraws]);
      });
    }
  };
