...
const UserBox = ({ people, setUserId }) => {
    ...
    const allOption = { id: 'all', title: 'همه' };
    const filteredPeople = query === ''
        ? [allOption, ...people] 
        : [allOption, ...people?.filter((person) => person.title.toLowerCase().includes(query.toLowerCase()))];
        ...
        <Combobox as="div" value={selectedPerson} onChange={handleSelectedPerson} className="parent-div">
            ...
            <div className="div-box relative">
                <Combobox.Input
                    ...
                    displayValue={(person) => person?.title ? person?.title : 'انتخاب نام استراتژی'}
                />
