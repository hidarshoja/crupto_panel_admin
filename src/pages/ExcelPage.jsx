...
export default function ExcelPage() {
  ...
  const checkExportStatus = async () => {
    try {
      ...
      const successfulData = response.data.data.filter(item => item.status === 100);
  ...
  const downloadExcelFile = async (exportId) => {
    try {
      const response = await axios.get(
        `${import.meta.env.VITE_API_BASE_URL2}/excel-exports/${exportId}/download`,
        {
          responseType: "blob",
          headers: getAuthHeaders(),
        }
      ...
      <ul>
        {successfulExports.map((item) => (
          <li key={item.id} className='bg-white p-4 rounded-md border border-gray-200 flex gap-2 items-center justify-between mb-3'>
            ...
            <button className='bg-blue-500 text-white px-4 py-2 rounded-md' onClick={() => downloadExcelFile(item.id)}>دانلود فایل</button>
          </li>
        ))}
      </ul>
    </div>
